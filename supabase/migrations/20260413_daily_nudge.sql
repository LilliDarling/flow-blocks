-- ============================================================
-- Dedup log for daily end-of-day nudge notifications.
...
BEGIN
  ...
  -- 10. daily_nudge_log older than 2 days
  DELETE FROM daily_nudge_log
  WHERE nudge_date < (CURRENT_DATE - INTERVAL '2 days');
  GET DIAGNOSTICS v_daily_nudge_log = ROW_COUNT;

  RETURN QUERY SELECT v_completions, v_done_items, v_energy_logs, v_pomo_sessions, v_oneoff_blocks, v_reminder_completions, v_push_notification_log, v_reminder_skips, v_energy_checkin_log, v_daily_nudge_log;
END;
$$;
