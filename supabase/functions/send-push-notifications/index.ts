...
//
// Web Push (PWA) only — native (Capacitor) clients schedule their own
// OS-level LocalNotifications and don't go through this function.
//
...
async function sendPush(
  ...
  if (!resp.ok) {
    ...
    console.error(`[push] web push ${resp.status}: ${errBody} endpoint=${sub.endpoint.slice(-20)}`);
  }
  return resp.status;
}
...
serve(async (req) => {
  ...
  // Fetch web push subscriptions. Native (Capacitor) clients are NOT served
  // here — they schedule their own OS-level LocalNotifications and never
  ...
  const getTimezone = (userId: string): string => tzByUser.get(userId) || 'UTC';

  const sendToAll = async (userId: string, payload: string): Promise<number> => {
    let count = 0;
